...
create_fractionalized_asset() {
    ...
    result=$(resim call-method $component create_fractionalized_asset $account2 660 15 --proofs $admin_badge_resource_address:#1#)
...
# account 1 buys 1
result=$(resim call-method $component buy_nft 0 1 $XRD:700)

resim set-default-account $account2 $account2_priv_key $account2_owner_badge

# account 2 buys 14
result=$(resim call-method $component buy_nft 0 14 $XRD:9500)

resim show $account2

# claim payout
result=$(resim call-method $component claim_payout $account)
